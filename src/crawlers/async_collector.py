...
class AsyncAPICollector:
    ...
    def __init__(self, data_query: Dict[str, Any], data_path: str, api_key: Optional[str] = None):
        """
    ...
    def load_rate_limit_from_config(self) -> None:
        ...
        try:
            ...
            logging.warning(f"{self.api_name}: Could not load rate limit from config: {e}")
            self.rate_limit = self.DEFAULT_RATE_LIMITS.get(self.api_name, 10.0)
            self.concurrency_limit = self.CONCURRENCY_LIMITS.get(self.api_name, 1)
    ...
    async def api_call_async(
        ...
        async with self.semaphore:
            ...
            for attempt in range(max_retries):
                try:
                    logging.debug(f"{self.api_name} API: Request {method} {url} (attempt {attempt + 1})")
                    ...
                    async with session.request(
                        ...
                        logging.debug(f"{self.api_name} API: Request successful (status {response.status})")

                        try:
                            data = await response.json()
                            return data
                        except asyncio.TimeoutError as e:
                            logging.error(f"{self.api_name} API: Response timeout while reading JSON")
                            if attempt < max_retries - 1:
                    ...
                    last_exception = e
                    logging.error(f"{self.api_name} API HTTP error {e.status}: {str(e)}")
            ...
            if last_exception:
                ...
                raise RuntimeError(f"{self.api_name} API: Failed after {max_retries} attempts")
    ...
    def savePageResults(self, page_data: Dict, page_num: int) -> None:
        ...
        page_file = os.path.join(self.datadir, f"page_{page_num:04d}.json")
